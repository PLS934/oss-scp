...
export class RecordQueryService {
  ...
  list(pluginId: string | undefined, sourceId: string | undefined, dataType: string | undefined, rawLimit: string | undefined, cursor: string | undefined): Promise<ListRecordsResult> {
    const limit = rawLimit === undefined ? undefined : /^\d+$/.test(rawLimit) ? Number(rawLimit) : Number.NaN;
    return this.query.listRecords({ pluginId: pluginId ?? '', sourceId: sourceId ?? '', dataType: dataType ?? '', ...(limit === undefined ? {} : { limit }), ...(cursor === undefined ? {} : { cursor }) });
  }
