...
export class RecordQueryController {
  ...
  @Get()
  async list(@Query('pluginId') pluginId?: string, @Query('sourceId') sourceId?: string, @Query('dataType') dataType?: string, @Query('limit') limit?: string, @Query('cursor') cursor?: string, @Query('page') page?: string, @Query('q') q?: unknown, @Query('filters') filters?: unknown, @Query('sort') sort?: unknown, @Query('direction') direction?: unknown) {
    try { return await this.records.list(pluginId, sourceId, dataType, limit, cursor, page, q, filters, sort, direction); } catch (error) { mapError(error); }
  }
  @Get(':id')
