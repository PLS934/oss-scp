...
export default function App({ menus: providedMenus }: { menus?: readonly MenuItem[] }) {
  ...
  return <div className="app-shell"><aside><p className="eyebrow">오픈소스 취약점 관리 플랫폼</p><h1><NavLink to="/">OSS-SCP</NavLink></h1>
    ...
    <div className="sidebar-meta"><p role="status" className={`status ${state}`}>{labels[state]}</p><p className="version">v{productVersion}</p></div></aside><div className="workspace"><header className="app-header"><span className="header-title">OSS-SCP</span><ThemePicker /></header><main><Routes>
