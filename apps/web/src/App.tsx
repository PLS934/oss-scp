...
export default function App({ menus: providedMenus }: { menus?: readonly MenuItem[] }) {
  ...
  return <div className="app-shell"><aside><p className="eyebrow">오픈소스 취약점 관리 플랫폼</p><h1><NavLink to="/">OSS-SCP</NavLink></h1>
    ...
    <div className="sidebar-meta"><label className="theme-picker" htmlFor="theme-preference">화면 테마<select id="theme-preference" value={theme} onChange={event => selectTheme(event.target.value as ThemePreference)}><option value="system">시스템 설정</option><option value="light">라이트</option><option value="dark">다크</option></select></label><p role="status" className={`status ${state}`}>{labels[state]}</p><p className="version">v{productVersion}</p></div></aside><main><Routes>
