...
export default function App({ menus: providedMenus, customViews = customPluginViews, authUser, onLogout }: { menus?: readonly MenuItem[]; customViews?: CustomPluginViewRegistry; authUser?: string; onLogout?: () => Promise<void> }) {
  ...
  return <div className="app-shell"><aside><p className="eyebrow">오픈소스 취약점 관리 플랫폼</p><h1><NavLink to="/">OSS-SCP</NavLink></h1>
    ...
    </aside><div className="workspace"><header className="app-header"><div className="header-actions"><p role="status" className={`status ${state}`}>{labels[state]}</p>{authUser ? <span className="auth-user">{authUser}</span> : null}{onLogout ? <button type="button" onClick={() => void onLogout()}>로그아웃</button> : null}<span className="version">v{productVersion}</span><ThemePicker /></div></header><main><Routes>
