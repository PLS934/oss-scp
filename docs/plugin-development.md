...
활성 로컬 CSV 카드의 **원본 내려받기**는 `GET /api/v1/plugins/:id/source-file`로 현재 등록 파일을 변환 없이 내려받습니다. 수집 시점의 보관본이 아니며 API JSON과 HTTP CSV는 다운로드를 제공하지 않습니다. 응답은 CSV attachment이고 `Cache-Control: no-store`를 적용합니다. 서버는 클라이언트 경로를 받지 않고 등록 정의만 사용하며 실제 파일이 설정 루트 내부의 일반 파일인지 재확인합니다. source `maxBytes`(생략 시 1 GiB)를 초과하면 413, 대상 없음·비활성·파일 누락·루트 이탈·읽기 불가는 내부 경로 없는 404로 응답합니다.
