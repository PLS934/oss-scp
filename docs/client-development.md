...
목록 상단의 `지금 동기화`는 현재 페이지나 데이터 종류만이 아니라 해당 플러그인의 활성 수집 대상 전체를 실행합니다. 접수와 실행 중에는 버튼을 비활성화하고 실행 상태만 polling합니다. 성공·부분 성공·실패가 확정되면 polling을 중단하고 cursor 이력과 위치를 초기화한 뒤 첫 묶음을 다시 조회합니다. 실패하거나 일부만 성공해도 기존 저장 데이터는 계속 표시됩니다.
