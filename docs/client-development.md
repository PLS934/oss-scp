...
`apps/web`의 React 시작 화면은 앱 제목과 서버 연결 상태를 표시합니다. 기본 인증 비활성화 설치는 로그인 없이 실행한다. 서버에서 인증을 활성화하면 웹은 업무 메뉴나 데이터를 요청하기 전에 세션을 확인하고, 세션이 없으면 LDAP/AD 로그인 화면만 표시한다. 상태 확인은 `/api/v1/health`를 호출하고 5초 안에 정상 응답이 없으면 실패로 표시합니다. 복구 후 페이지를 새로고침하면 다시 확인합니다.
...
인증 활성화 배포에서는 로그인 성공 후 HttpOnly session 쿠키를 사용한다. 브라우저 JavaScript나 localStorage에는 비밀번호와 session ID를 저장하지 않는다. 새로고침하면 `/api/v1/auth/session`으로 세션을 복구하고, 보호 API가 401을 반환하면 기존 앱을 unmount한 뒤 세션을 다시 확인한다. 로그아웃하면 화면 데이터를 폐기하고 로그인 화면으로 돌아간다. 운영 쿠키는 Secure이므로 [서버 가이드의 LDAP/AD 로그인 절차](server-development.md#ldapactive-directory-로그인)에 따라 HTTPS 동일 출처로 제공해야 한다.
...
API 계층 자체는 UI 상태, 캐시·자동 재시도·timeout을 제공하지 않습니다. 번호형 조회는 `listRecords({ ...scope, page: 1, limit: 20 })` 또는 `listNumberedRecords`로 요청합니다. 번호형 응답의 page/pageSize/totalItems/totalPages/hasNextPage와 items 수를 검증하며 nextCursor를 요구하지 않습니다. 선택적 단일 정렬은 `sort: { field: 'score', direction: 'desc' }`로 전달합니다. 기존 cursor 호출은 고정 정렬로 유지합니다.
...
첫 요청은 page=1, limit=20을 사용합니다. 화면에서 20·50·100·200건 중 페이지 크기를 선택하면 1페이지부터 다시 조회합니다. 처음·이전·최대 10개 페이지 번호·다음·마지막으로 직접 이동하고 현재 표를 교체합니다. 번호는 1~10, 11~20처럼 현재 페이지를 포함한 구간으로 표시하며 현재 버튼에는 aria-current=page가 적용됩니다. 메뉴 이동이나 크기 변경 중인 요청은 취소하고 늦은 응답을 무시합니다. 빈 결과는 전체 0건과 비활성 이동 버튼으로 표시하며, 데이터 감소로 요청 페이지가 범위를 벗어나면 서버가 반환한 마지막 유효 페이지로 보정합니다.

목록은 로딩, 미수집, 정상 빈 결과, 수집 중, 마지막 실행 실패·부분 완료와 API 조회 실패를 구분합니다. 수집 중·실패·부분 완료 응답에 저장된 items가 있으면 상태 안내와 함께 기존 데이터를 계속 표시합니다. 조회 실패의 `다시 시도`는 실패한 조회 범위·페이지·크기·검색·필터·정렬을 재요청합니다. 개인 컬럼 선택·순서·너비, 다중 정렬, 중첩 object·array 고급 표시와 사용자 정의 React 화면은 후속 범위입니다.

전체 건수 옆에는 `list.sorts`에 선언된 필드를 뱃지로 표시한다. 같은 뱃지를 누르면 오름차순, 내림차순, 해제 순으로 바뀌며 다른 뱃지를 누르면 기존 정렬을 해제하고 새 필드의 오름차순을 적용한다. 정렬 변경은 현재 검색·필터·페이지 크기를 유지하고 첫 페이지부터 즉시 조회한다.
...
목록 제목 옆의 동기화 아이콘은 현재 페이지나 데이터 종류만이 아니라 해당 플러그인의 활성 수집 대상 전체를 실행합니다. 아이콘 옆에는 마지막 동기화 시각과 짧은 실행 상태를 표시합니다. 접수와 실행 중에는 아이콘을 회전시키고 버튼을 비활성화한 채 실행 상태만 polling합니다. 성공·부분 성공·실패가 확정되면 polling을 중단하고 cursor 이력과 위치를 초기화한 뒤 첫 묶음을 다시 조회합니다. 실패하거나 일부만 성공해도 기존 저장 데이터는 계속 표시됩니다.

화면은 plugin sync capability 응답으로 활성 대상 없음, 이미 실행 중, 권한 없음과 마지막 성공 시각을 구분합니다. route를 바꾸면 이전 polling과 목록 요청을 취소하고 늦은 응답을 폐기합니다. 브라우저를 새로고침해 로컬 요청 ID를 잃어도 plugin 상태 endpoint의 현재 run으로 진행 상태를 복원합니다.
...
서버 연결 상태와 제품 버전은 상단 헤더에서 테마 버튼 옆에 표시합니다. 사이드바에는 제품 이름과 메뉴 탐색을 둡니다.
상단의 태양·달 버튼은 현재 테마를 표시하며 누르면 반대 테마로 즉시 전환합니다. 저장된 선택이 없을 때만 시스템 설정을 따르고, 직접 전환하면 그 선택을 저장해 다음 방문에도 복원합니다.
## 목록 검색과 필터

검색·필터 UI는 메뉴의 선택적 `list.query` 선언으로 표시한다. 선언을 생략한 메뉴는 기존 목록을 유지한다. 검색어와 선택값·숫자 범위·UTC 날짜 범위를 입력한 뒤 **적용** 또는 Enter로 함께 확정한다. 입력 중에는 이전 결과와 미적용 안내를 표시하고, 새 조건 조회 중에는 이전 조건의 행을 제거한다.

검색·필터 적용과 초기화는 번호형 1페이지부터 조회한다. 전체 건수·페이지 수는 일치 결과 기준이다. 초기화와 메뉴 전환은 입력 및 적용 조건을 비우고, 조회 실패의 **다시 시도**는 같은 조건을 유지한다. 활성 조건의 빈 결과는 검색·필터 결과 없음과 초기화 버튼을 표시하며 수집 진행·부분 완료·실패 상태도 함께 안내한다. 날짜 입력의 시작일은 UTC 00:00 포함, 종료일은 다음 날 UTC 00:00 미포함이다.

`pnpm test:browser:search`는 실제 PostgreSQL·MySQL 각각에서 샘플 선언, 화면 입력, API, 전체 데이터 조회를 연결해 검증한다. `SEARCH_DB_TYPE=postgres` 또는 `mysql`로 한 DB만 선택할 수 있다. CI의 두 DB 작업에서도 이 명령을 실행하고 `test-results/record-search-*.json`에 1,000건 fixture의 EXPLAIN과 조회 시간을 남긴다. 이는 기능 검증용 측정이며 운영 성능 보장은 아니다.
