...
API 계층 자체는 UI 상태, 캐시·자동 재시도·timeout을 제공하지 않습니다. 번호형 조회는 `listRecords({ ...scope, page: 1, limit: 20 })` 또는 `listNumberedRecords`로 요청합니다. 번호형 응답의 page/pageSize/totalItems/totalPages/hasNextPage와 items 수를 검증하며 nextCursor를 요구하지 않습니다. 기존 cursor 호출은 유지합니다. 이전 cursor 생성과 검색·필터·사용자 지정 정렬은 제공하지 않습니다.
...
목록은 로딩, 미수집, 정상 빈 결과, 수집 중, 마지막 실행 실패·부분 완료와 API 조회 실패를 구분합니다. 수집 중·실패·부분 완료 응답에 저장된 items가 있으면 상태 안내와 함께 기존 데이터를 계속 표시합니다. 조회 실패의 `다시 시도`는 실패한 조회 범위·페이지·크기를 재요청하며 성공 전까지 마지막 성공 목록과 위치를 유지합니다. 검색·필터·사용자 지정 정렬, 개인 컬럼 선택·순서·너비, 중첩 object·array 고급 표시와 사용자 정의 React 화면은 후속 범위입니다.
