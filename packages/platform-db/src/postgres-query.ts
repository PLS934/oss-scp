    ...
    try {
      return await connection.withClient(async client => {
        ...
        try {
          ...
          const recordsResult = await client.query<RecordRow>(`SELECT id, plugin_id, source_id, data_type, external_key_type, external_key, source_values, first_seen_at, last_seen_at FROM platform_records WHERE plugin_id=$1 AND source_id=$2 AND data_type=$3${filtered.sql}${boundary} ORDER BY last_seen_at DESC, id ASC LIMIT $${parameters.length - (numbered ? 1 : 0)}${numbered ? ` OFFSET $${parameters.length}` : ''}`, parameters);
