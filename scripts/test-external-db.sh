...
export PLATFORM_DB_PASSWORD=external-test-password
export PLATFORM_DB_TLS_MODE=disable
cleanup() {
  result=$?
  docker compose -p "$project" -f compose.external-db.yaml down --remove-orphans >/dev/null 2>&1 || true
  docker rm -f "$database" >/dev/null 2>&1 || true
  exit "$result"
}
trap cleanup EXIT
docker run -d --name "$database" -e POSTGRES_DB=oss_scp -e POSTGRES_USER=oss_scp_app \
  -e POSTGRES_PASSWORD="$PLATFORM_DB_PASSWORD" -p 127.0.0.1::5432 postgres:17.6-bookworm >/dev/null
...
docker compose -p "$project" -f compose.external-db.yaml up --build -d --wait --wait-timeout 90 api
test "$(curl --fail --silent --show-error --max-time 5 "http://127.0.0.1:${API_PORT}/api/v1/ready")" = '{"status":"ready"}'
docker compose -p "$project" -f compose.external-db.yaml run --rm api node node_modules/@oss-scp/platform-db/dist/migrate-cli.js | grep -q '1개 적용'
docker compose -p "$project" -f compose.external-db.yaml down --remove-orphans
