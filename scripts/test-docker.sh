...
cleanup() {
  ...
  docker rm -f "$standalone" "$unresponsive" "$invalid_revision" >/dev/null 2>&1 || true
...
docker compose -p "$project" run --rm api node node_modules/@oss-scp/platform-db/dist/migrate-cli.js | grep -q '5개 적용'
...
test "$(docker compose -p "$project" exec -T postgres psql -U oss_scp_app -d oss_scp -Atc 'select count(*) from oss_scp_schema_migrations')" = 5
