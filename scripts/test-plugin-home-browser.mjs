...
export async function checkPluginHome(page, url, menus, root) {
  ...
  await page.reload();
  await expect(main.getByRole('link', { name: '자산 플러그인 설정 상세 보기' })).toHaveAttribute('href', `/plugins/${menus[0].pluginId}`);
  ...
  const disabledCard = main.locator('.plugin-card').filter({ hasText: '비활성 CSV' });
  await expect(disabledCard.getByRole('link', { name: '비활성 CSV 설정 상세 보기' })).toHaveAttribute('href', '/plugins/disabled');
  await expect(disabledCard.getByRole('link', { name: /데이터 보기/ })).toHaveCount(0);
  const noMenuCard = main.locator('.plugin-card').filter({ hasText: '메뉴 없는 플러그인' });
  await expect(noMenuCard.getByRole('link', { name: '메뉴 없는 플러그인 설정 상세 보기' })).toHaveAttribute('href', '/plugins/no-menu');
  await expect(noMenuCard.getByRole('link', { name: /데이터 보기/ })).toHaveCount(0);
  ...
  await expect(main.locator('.plugin-title').getByRole('link')).toHaveCount(3);
