...
docker compose -p "$bundled_project" -f compose.mysql.yaml run --rm api node node_modules/@oss-scp/platform-db/dist/migrate-cli.js | grep -q '8개 적용'
...
test "$(docker compose -p "$bundled_project" -f compose.mysql.yaml exec -T mysql mysql -N -uoss_scp_app -p"$PLATFORM_DB_PASSWORD" oss_scp -e 'select count(*) from oss_scp_schema_migrations')" = '8'
...
docker compose -p "$external_project" -f compose.external-db.mysql.yaml -f compose.external-db.mysql.secret.yaml run --rm api node node_modules/@oss-scp/platform-db/dist/migrate-cli.js | grep -q '8개 적용'
...
test "$(docker exec "$external_database" mysql -N -uoss_scp_app -p"$PLATFORM_DB_PASSWORD" oss_scp -e 'select count(*) from oss_scp_schema_migrations')" = '8'
