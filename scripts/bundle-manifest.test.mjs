...
const baseImages = [
  { name: 'api', reference: 'oss-scp-api:0.1.0', digest, configDigest: `sha256:${'e'.repeat(64)}` },
  { name: 'web', reference: 'oss-scp-web:0.1.0', digest: `sha256:${'c'.repeat(64)}`, configDigest: `sha256:${'f'.repeat(64)}` },
];
...
test('PostgreSQL 포함 manifest는 DB 이미지를 요구한다', () => {
  ...
  const manifest = createBundleManifest({ productVersion: '0.1.0-rc.1', gitRevision: revision, variant: 'postgresql', createdAt: '2026-09-13T00:00:00Z', images: [...baseImages, { name: 'postgresql', reference: 'postgres:17.6-bookworm', digest: `sha256:${'d'.repeat(64)}`, configDigest: `sha256:${'1'.repeat(64)}` }] });
...
test('잘못된 schema, version, revision, variant와 digest를 거부한다', () => {
  ...
  for (const change of [
    ...
    { images: [{ ...baseImages[0], configDigest: 'bad' }, baseImages[1]] },
