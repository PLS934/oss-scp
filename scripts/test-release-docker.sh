...
docker compose -p "$project" -f "$first_compose" run --rm api node node_modules/@oss-scp/platform-db/dist/migrate-cli.js | grep -q '7개 적용'
...
test "$(docker compose -p "$project" -f "$second_compose" exec -T postgres psql -U oss_scp_app -d oss_scp -Atc 'SELECT count(*) FROM oss_scp_schema_migrations')" = '7'
test "$(curl --fail --silent --show-error --max-time 5 "http://127.0.0.1:${API_PORT}/api/v1/ready")" = '{"status":"ready"}'
