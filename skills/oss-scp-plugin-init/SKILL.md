...
기존 대화나 파일에서 확인 가능한 값은 재질문하지 않는다. 실제 원천 샘플, 안정적인 유일키, 표시할 필드, 플러그인 ID, 새 출력 폴더, 대상 플랫폼 버전과 검증 환경을 확인한다. 정보가 없으면 예제임을 명시하고 초기 설정을 만들 수 있지만 실제 연동 완료로 보고하지 않는다.
...
생성 폴더, 선택한 source, 예제로 남은 값, 실제 실행한 검증과 결과, 첫 수집에 필요한 다음 단계를 알린다. 첫 수집과 배포 안내는 [작성 계약](references/authoring.md#첫-수집)에서 확인한다.
