...
- [호스트의 secret 파일 경로·권한은 비루트 컨테이너와 다를 수 있음] → 외부 DB용 override가 Compose secret으로 고정 경로에 읽기 전용 제공하고, 로컬 Compose가 보존하는 source 권한상 UID 1000의 읽기 권한이 필요함을 문서화하며, 직접 값 방식과 동시에 전달하지 않는다.
