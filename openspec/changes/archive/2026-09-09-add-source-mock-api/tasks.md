...
Linux Docker는 중첩 볼륨의 빈 mountpoint 디렉터리를 호스트 bind mount에 만들 수 있다. 기존 ENOENT 검사는 이를 실패로 오인했다. 호스트 dist가 없거나 비어 있고 컨테이너 dist의 실제 mount type이 volume인지 확인하도록 보정한다. 기존 검사에서 발생한 PR CI 실패는 이 조건 오류이며 기능 검사 단계는 통과했다.
