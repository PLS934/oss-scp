...
검색·필터 결과가 없을 때 표시하던 조건 초기화 버튼을 제거하고 안내 문구만 남겼다. 사용자는 검색창 X와 개별 조건 배지 X로 조건을 해제할 수 있다. 웹 테스트 123개와 타입 검사, OpenSpec strict 검증을 통과했으며 PostgreSQL·MySQL 브라우저 시나리오에서 버튼 미표시와 검색 지우기를 통한 전체 결과 복귀를 확인했다.
