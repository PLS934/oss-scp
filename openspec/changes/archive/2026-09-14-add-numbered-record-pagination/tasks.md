...
- [x] 4.4 #87/#74의 준비된 브랜치를 임시 통합 환경에서 조합해 행 이동·번호형 조회·두 테마의 표시와 접근성을 검증한다. 각 이슈 PR diff에 다른 이슈 구현이 섞이지 않았는지 확인한다.
- [x] 4.5 최신 main을 다시 확인하고 저장소 PR 템플릿에 #86 해결 범위·실제 검증·미검증 항목을 작성한다. 별도 PR 하나를 게시하고 본문 및 base diff를 확인한다.

검증 기록: 웹 87개·API 25개·platform-db 150개(실제 PostgreSQL/MySQL 포함), 전체 build/typecheck/lint, Docker PostgreSQL 기반 browser 회귀가 통과했다. PR #102를 main 대상으로 게시했다. #100·#101과 임시 통합 브랜치에서 겹치는 renderer/CSS/browser test를 조정한 뒤 전체 build, 웹 102개 테스트와 browser 회귀(번호형 이동·행 링크·두 테마 포함)를 통과했다. 각 PR은 해당 이슈 코드만 유지한다. 원격 CI 결과는 PR checks에서 별도로 확인한다.
