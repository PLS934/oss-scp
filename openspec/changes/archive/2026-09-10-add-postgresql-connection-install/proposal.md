...
- 기본 `compose.yaml` 설치에 영속 볼륨을 가진 PostgreSQL을 포함한다. 이미 실행 중인 외부 DB는 독립 `compose.external-db.yaml`로 API·웹만 시작하고 환경변수/secret 파일로 연결하며, oss-scp는 외부 DB의 생성·기동·종료·삭제를 관리하지 않는다.
