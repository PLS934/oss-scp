...
홈은 활성 플러그인의 검증된 메뉴에만 데이터 보기 링크를 SHALL 제공해야 한다. 조회 링크는 플러그인 제목 옆 오른쪽 화살표 아이콘으로 표시하고 복수 메뉴는 접근성 이름과 툴팁으로 구분해야 한다(MUST). 메뉴 그룹과 메뉴 이름을 데이터 출처 위에 순서대로 표시한다. 복수 메뉴의 동일한 그룹 이름은 중복 표시하지 않는다.
