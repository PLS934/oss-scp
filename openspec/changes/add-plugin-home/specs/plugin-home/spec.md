...
홈은 기본 순과 이름 순을 선택하는 정렬 컨트롤을 SHALL 제공해야 한다. 기본 순은 registry 응답 순서이며 이름 순은 표시 이름의 한국어 locale 오름차순으로 영문 대소문자를 구분하지 않고 숫자를 자연 순서로 비교한다.

#### Scenario: 정렬 전환
- **WHEN** 사용자가 이름 순을 선택한다
- **THEN** 모든 플러그인을 이름 순으로 표시하고 같은 이름은 기존 순서를 유지하며 기본 순으로 변경하면 원래 순서로 복원한다
