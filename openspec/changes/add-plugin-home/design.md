...
제목 오른쪽에 기본 순·이름 순 선택기를 제공한다. 초기값은 기존 registry 순서를 유지하는 기본 순이다. 이름 순은 한국어 locale의 Intl.Collator와 numeric·base 비교로 오름차순 정렬하며 같은 이름은 기존 순서를 유지한다. 원본 배열을 변경하거나 API를 다시 요청하지 않는다.
