...
사용자 요청에 따라 빈 설명 영역과 현재 CSV 원본 안내 문구를 제거한다. 메뉴 이름은 데이터 출처 위에 표시하고, 데이터 조회는 플러그인 제목 옆 링크 아이콘으로 제공한다. 복수 메뉴는 메뉴별 접근성 이름·툴팁을 가진 아이콘으로 구분한다. 로컬 CSV 다운로드 아이콘은 파일명 오른쪽에 놓고 접근성 이름·키보드 포커스를 유지한다.
