...
수집 결과를 PostgreSQL에 저장할 수 있게 되었지만, 현재는 원천 장애와 무관하게 저장된 데이터를 확인할 공통 조회 경로가 없다. 사용자가 마지막으로 저장된 목록·상세와 수집 상태를 안정적으로 확인할 수 있도록 DB 제품에 종속되지 않는 최소 조회 계약과 API가 필요하다.
