...
클라이언트는 서버가 기동 시 외부 설정 루트에서 검증한 플러그인 registry의 메뉴 산출물만 사용하여 메뉴 제목, 아이콘, 그룹, 순서와 경로를 표시해야 한다(SHALL). 같은 산출물은 메뉴 조회 범위에 대응하는 검증된 기본 목록 columns의 `key`, `label`, `type`을 제공해야 한다(SHALL). 브라우저에서 플러그인 구조를 생성·수정하거나 원시 Git 설정을 다시 검증하지 않아야 한다(MUST).

#### Scenario: 등록된 메뉴 표시
- **WHEN** 서로 다른 그룹·순서·경로와 기본 목록을 선언한 복수 외부 플러그인이 서버의 검증된 메뉴 응답에 포함된다
- **THEN** 클라이언트는 각 선언의 제목·아이콘·그룹·경로를 사용해 메뉴를 표시하고 해당 조회 범위의 검증된 목록 columns를 사용할 수 있다
...
- **WHEN** 운영자가 유효한 메뉴와 기본 목록을 가진 새 플러그인을 외부 Git registry에 추가하고 검증 후 서버를 재기동한다
- **THEN** 클라이언트 코어나 플랫폼 이미지를 다시 빌드하지 않아도 새 메뉴와 목록 메타데이터가 제공된다

#### Scenario: registry 응답의 정보 경계
- **WHEN** 클라이언트가 검증된 메뉴와 기본 목록 산출물을 조회한다
- **THEN** 응답은 조회에 필요한 식별자·표시 속성·목록 column만 포함하고 원천 설정, Connection 값과 서버 모듈 경로를 포함하지 않는다
