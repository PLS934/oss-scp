...
홈은 활성 플러그인의 검증된 메뉴에만 데이터 보기 링크를 SHALL 제공해야 한다. 조회 링크는 플러그인 제목 옆 오른쪽 화살표 아이콘으로 표시하고 복수 메뉴는 접근성 이름과 툴팁으로 구분해야 한다(MUST). 플러그인 이름 바로 아래에 `{메뉴 그룹} > {메뉴 이름}` 형태의 메뉴 위치를 표시하고 개별 메뉴 그룹·메뉴 이름 행은 제공하지 않는다. 복수 메뉴는 각 메뉴 위치를 별도로 표시한다.
