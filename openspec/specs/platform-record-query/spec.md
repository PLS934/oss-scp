...
- **WHEN** 요청한 묶음의 요약 데이터가 전체 응답 크기 한도를 넘긴다
...
PostgreSQL 구현은 공통 조회 계약을 SHALL 구현하고 조회 중 외부 원천 API·DB·파일을 호출해서는 안 된다. 목록은 offset 없이 마지막 `lastSeenAt/id`를 기준으로 다음 묶음을 읽어야 한다.
