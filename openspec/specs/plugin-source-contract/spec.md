...
- **WHEN** 계약 버전이나 필수 식별·필드 표시명·기본 목록·상세 정보가 없거나 source·가공 모듈 경로가 플러그인 디렉터리 밖을 참조한다
...
- **WHEN** 등록된 플러그인의 가공 모듈이 없거나 JavaScript가 아니거나, 활성 플러그인의 가공 모듈이 요구된 export를 제공하지 않는다
- **THEN** 플랫폼은 다른 경로의 파일을 대신 실행하거나 코드를 변환하지 않고 해당 플러그인의 검증을 실패한다

#### Scenario: 등록된 샘플 중 필수 가공 계약 누락
- **WHEN** sample1, sample2 또는 CSV를 포함한 등록 플러그인 중 하나가 가공 모듈, 표시명이 포함된 데이터 정의 또는 기본 목록·상세 선언을 제공하지 않는다
- **THEN** 플랫폼은 누락된 플러그인과 필드를 보고하고 전체 설정 검증을 성공으로 표시하지 않는다

#### Scenario: 기존 설정과 선택 정보
- **WHEN** 설명과 enabled가 없는 기존 플러그인을 검증한다
- **THEN** 기존 활성 수집 동작을 유지하며 설명은 생략할 수 있다

#### Scenario: 비활성 등록
- **WHEN** 유효한 플러그인이 enabled false를 선언한다
- **THEN** 등록 요약은 비활성으로 표시하고 수집 정의·메뉴를 생성하거나 transform을 로딩하지 않는다

#### Scenario: 메뉴 생략
- **WHEN** 유효한 활성 플러그인이 menu를 생략한다
- **THEN** 등록 요약과 수집 정의를 제공하며 조회 메뉴는 생성하지 않는다
