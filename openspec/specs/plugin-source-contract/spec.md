...
플랫폼은 플러그인의 계약 버전, 고유 식별자, 표시 이름, 릴리스 버전, 상대 source 파일 경로, 상대 가공 모듈 경로, 데이터 정의, 단일 메뉴 선언과 데이터 종류별 기본 목록 선언을 외부 설정 루트의 `plugin.json`에서 선언할 수 있게 SHALL 제공해야 한다. 메뉴 선언은 제목, 지원 아이콘, 그룹, 정렬 순서, 절대 경로와 대상 데이터 종류를 포함해야 한다. 데이터 정의는 하나 이상의 데이터 종류, 각 종류의 필드와 타입, 사람이 읽을 수 있는 필수 표시명, 필수 여부, 유일키 및 선택적 관계를 포함해야 한다. 각 데이터 종류의 기본 목록은 비어 있지 않은 최상위 scalar 필드 참조를 순서대로 포함해야 하며, 가공·화면 외의 지원하지 않는 설정을 필수로 요구하지 않아야 한다. 가공 모듈은 운영 배포 전에 생성된 JavaScript여야 하며(MUST), 플랫폼은 그 모듈의 요구 export를 기동 전에 검증해야 한다.

#### Scenario: 유효한 sample1 플러그인
- **WHEN** 운영자가 지원 계약 버전, 유효한 상대 source·사전 빌드 JavaScript 가공 모듈 경로, 표시명이 포함된 완전한 데이터 정의, 유효한 메뉴와 기본 목록 선언을 가진 sample1 `plugin.json`을 외부 설정 루트에서 검증한다
- **THEN** 플랫폼은 해당 플러그인 선언을 실행 가능한 플러그인으로 승인하고 내부 수집 정의에 가공 모듈, 데이터 정의, 메뉴와 기본 목록을 포함한다

#### Scenario: 등록된 sample2와 CSV 플러그인
- **WHEN** sample2와 CSV 샘플 플러그인이 외부 registry에 등록되어 있고 각각 유효한 가공 모듈, 표시명이 포함된 데이터 정의, 메뉴와 기본 목록 선언을 제공한다
- **THEN** 플랫폼은 source 형식과 무관하게 각 플러그인의 내부 정의에도 가공 모듈, 데이터 정의, 메뉴와 기본 목록을 포함한다

#### Scenario: 잘못된 플러그인 선언
- **WHEN** 계약 버전이나 필수 식별·메뉴·필드 표시명·기본 목록 정보가 없거나 source·가공 모듈 경로가 플러그인 디렉터리 밖을 참조한다
...
- **WHEN** 유일키 또는 메뉴가 선언되지 않은 필드를 가리키거나 관계 또는 메뉴가 존재하지 않는 데이터 종류를 참조하거나 기본 목록이 유효하지 않은 필드를 참조한다
...
- **WHEN** sample1, sample2 또는 CSV를 포함한 등록 플러그인 중 하나가 가공 모듈, 표시명이 포함된 데이터 정의, 메뉴 또는 기본 목록 선언을 제공하지 않는다
